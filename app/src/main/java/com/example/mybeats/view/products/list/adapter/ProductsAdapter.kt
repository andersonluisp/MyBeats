...
class ProductsAdapter(val onItemClick: (Product) -> Unit) :
    ListAdapter<Product, ProductsAdapter.ProductsAdapterViewHolder>(ProductsDiffUtil()) {
    ...
    inner class ProductsAdapterViewHolder(
        ...
        fun bind(item: Product) {
            ...
            binding.tvProductPrice.text =
                binding.root.context.resources.getString(R.string.BRL, item.price)
            binding.tvReviews.text = binding.root.context.resources.getQuantityString(
    ...
    fun filterList(filterText: String?) {
        ...
        if (filterText.isNullOrEmpty()) {
            ...
            val result = productsList.filter {
                it.model.lowercase(Locale.getDefault())
